...
class NationalId
{
	...
	const CA_OHIP_VALID_LENGTH_HEALTH_NUMBER	=  10;
	const CA_OHIP_MIN_LENGTH_VERSION_CODE		=   0;
	const CA_OHIP_MAX_LENGTH_VERSION_CODE		=   2;
	const CA_OHIP_INVALID_LENGTH_VERION_NUMBER	= 101;
	const CA_OHIP_INVALID_LENGTH_HEALTH_NUMBER	= 102;
	const CA_OHIP_INVALID_VERSION_CODE		= 103;
			...
			case 'CA':
				switch( strtoupper( $type ) )
				{
					case 'SIN':
						...
						break;
					case 'OHIP':
						$result = $this->validate_CA_OHIP( $id_number );
						break;
					default:
						break;
				}
	...
	/**
	 * Format a Canadian Ontario Health Number.
	 *
	 * @author André Fortin <andre.v.fortin@gmail.com>
	 */
	private function format_CA_OHIP( $card_number )
	{
		$is_valid	= false;
		$error_code	= 0;
		$location	= '';
		...
		// convert to a string and strip any non-numeric characters.
		$card_number = (string)$card_number;
		$card_number = preg_replace("/[^0-9,A-Z]/", "", $card_number );
		
		$formatted_number  = substr( $card_number, 0, 4 );
		$formatted_number .= '-';
		$formatted_number .= substr( $card_number, 4, 3 );
		$formatted_number .= '-';
		$formatted_number .= substr( $card_number, 7, 3 );
		
		if( strlen( $card_number ) > self::CA_OHIP_VALID_LENGTH_HEALTH_NUMBER )
		{
			$formatted_number .= '-';
			$formatted_number .= substr( $card_number, 10, 2 );
		}
	...
	/**
	 * Generate a valid looking Canadian Ontario Health Number.
	 *
	 * The first digit of a SIN indicates province of registration.
	 *
	 * @author André Fortin <andre.v.fortin@gmail.com>
	 *
	 */
	private function generate_CA_OHIP()
	{
		$ohip = '';
		
		// Generate the Ontario Health Number, formerly OHIP number.
		for( $digits = 0; $digits < self::CA_OHIP_VALID_LENGTH_HEALTH_NUMBER; $digits++ )
		{
			$ohip .= rand( 0, 9 );
		}
		
		// Now generate the Version Code.
		for( $letters = 0; $letters < self::CA_OHIP_MAX_LENGTH_VERSION_CODE; $letters++ )
		{
			$int = rand( 0, 23 );
	...
	/**
	 * Validate a Canadian Ontario Health Number.
	 *
	 * @author André Fortin <andre.v.fortin@gmail.com>
	 */
	private function validate_CA_OHIP( $card_number )
	{
		$is_valid	= false;
		$error_code	= 0;
		$location	= 'ON';
		...
		// convert to a string and strip any non-numeric characters.
		$card_number = strtoupper( (string)$card_number );
		$card_number = preg_replace("/[^0-9,A-Z]/", "", $card_number );
		
		// Validate the Health Number length.
		$health_number = substr( $card_number, 0, self::CA_OHIP_VALID_LENGTH_HEALTH_NUMBER );
		if( self::CA_OHIP_VALID_LENGTH_HEALTH_NUMBER == strlen( $health_number ) )
		{
			// Validate the version code if present.
			$version_code = substr( $card_number, 10, 2 );
			if( strlen( $version_code ) >= self::CA_OHIP_MIN_LENGTH_VERSION_CODE && 
				strlen( $version_code ) <= self::CA_OHIP_MAX_LENGTH_VERSION_CODE )
			{
				...
				if( false === $result )
				{
					...
					if( false === $result )
					{
						...
						$is_valid = false;
						$error_code = self::CA_OHIP_INVALID_VERSION_CODE;
					}
				} else {
					$is_valid = false;
					$error_code = self::CA_OHIP_INVALID_VERSION_CODE;
				...
				$is_valid = false;
				$error_code = self::CA_OHIP_INVALID_LENGTH_VERION_CODE;
			}

		} else {
			$is_valid = false;
			$error_code = self::CA_OHIP_INVALID_LENGTH_HEALTH_NUMBER;
		}
		
		return array( "success"=>$is_valid, "code"=>$error_code, "id_number"=>$card_number );
	}	
